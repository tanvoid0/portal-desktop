...
export const POST: RequestHandler = async ({ params, request }) => {
	try {
		...
		if (command === 'generate_device_passcode' && args.request && args.request.device_info) {
			try {
				const deviceInfo = typeof args.request.device_info === 'string' 
		...
		return json(
			{
				error: 'Tauri commands can only be called from the Tauri frontend. For external device access, a Tauri HTTP server plugin is required.',
				command,
				args,

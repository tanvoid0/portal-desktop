	...
	<div class="space-y-6">
			...
			<CardContent class="space-y-4">
				<div>
					...
					<p class="text-xs text-muted-foreground mt-2">
						Use <code class="bg-muted px-1 rounded">${'{paramName}'}</code> for parameter placeholders.
							...
							<div class="flex items-center justify-between p-4">
								<div class="space-y-1">
									<div class="flex items-center gap-2">
										<code class="font-mono text-sm font-medium bg-muted px-2 py-0.5 rounded">
											${'{' + param.name + '}'}
										</code>
		...
		{#if formData.name || formData.command}
			<Card>
				...
				<CardContent>
					<pre class="bg-muted rounded-lg p-4 text-xs font-mono overflow-x-auto max-h-60">{JSON.stringify(formData, null, 2)}</pre>
				</CardContent>
			</Card>
		{/if}

...
<script lang="ts">
  ...
  const filteredDaemonSets = $derived(
    $cloudStore.resources[ResourceType.DAEMONSET].filter((daemonset) => {
      const matchesSearch =
        !searchQuery || daemonset.name.toLowerCase().includes(searchQuery.toLowerCase());
...
<div class="space-y-6">
  <div class="flex items-center justify-between">
    <div>
      ...
      <p class="mt-1 text-muted-foreground">Manage daemon workloads that run on every node</p>
    </div>
  ...
  <div class="flex gap-4">
    <Input placeholder="Search DaemonSets..." bind:value={searchQuery} class="max-w-sm" />

...
<script lang="ts">
  ...
  const filteredDaemonSets = $derived(
    $cloudStore.resources[ResourceType.DAEMONSET].filter((daemonset) => {
      const matchesSearch =
        !searchQuery ||
        daemonset.name.toLowerCase().includes(searchQuery.toLowerCase());
  ...
  const daemonSetStats = $derived({
    ...
    running: $cloudStore.resources[ResourceType.DAEMONSET].filter(
      (ds: any) => ds.status === "running",
    ).length,
    pending: $cloudStore.resources[ResourceType.DAEMONSET].filter(
      (ds: any) => ds.status === "pending",
    ).length,
    failed: $cloudStore.resources[ResourceType.DAEMONSET].filter(
      (ds: any) => ds.status === "failed",
    ).length,
  });
  ...
  const tableNav = useTableNavigation({
    ...
    onActivate: (index) => {
      ...
      if (daemonset) {
        goto(
          `/cloud/workloads/daemonsets/${daemonset.name}?namespace=${daemonset.namespace}`,
        );
      }
    },
  ...
  const resourceActions = useResourceActions({
    ...
    handlers: {
      onDescribe: (resource) => {
        goto(
          `/cloud/workloads/daemonsets/${resource.name}?namespace=${resource.namespace}`,
        );
      },
      onEdit: (resource) => {
        goto(
          `/cloud/workloads/daemonsets/${resource.name}?namespace=${resource.namespace}&tab=yaml`,
        );
      },
      onYaml: (resource) => {
        goto(
...
<div class="container mx-auto space-y-6 p-6">
  <div class="flex items-center justify-between">
    <div>
      ...
      <p class="mt-1 text-muted-foreground">
        Manage daemon workloads that run on every node
      </p>
    </div>
  ...
  <div class="k8s-navigable-table" data-selected-index={tableNav.selectedIndex}>
    <BaseResourceTable
      ...
      filteredEmptyMessage="No DaemonSets match your filters"

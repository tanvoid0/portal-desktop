...
<div class="space-y-6 p-6">
  {#if isLoading}
    <PageLoading message="Loading pod..." />
  {:else if error}
    <PageError title="Failed to load pod" message={error} onRetry={loadPod} />
  {:else if pod}
    <!-- Header -->
      ...
      <TabsContent value="yaml" class="space-y-4">
        <Card>
          ...
          <CardContent>
            ...
            {:else if yaml}
              <pre
                class="max-h-[600px] overflow-auto whitespace-pre-wrap rounded-lg bg-muted p-4 font-mono text-xs">{yaml}</pre>
            {:else}
          ...
          <Button
            onclick={async () => {
              ...
              try {
                ...
                portForwardMessage = null;
                const result = (await podApi.startPortForward({
                  namespace: pod.namespace,
                  podName: pod.name,

...
<script lang="ts">
  ...
  const isLoading = $derived($cloudStore.loading.resources[ResourceType.POD] ?? false);
</script>
...
<WorkloadListShell
  title="Pods"
  description="Namespace: {$cloudStore.selectedNamespace || 'All'}"
  resourceType={ResourceType.POD}
  {isLoading}
  itemCount={filteredPodsLength}
  emptyTitle="No pods found"
  emptyDescription="No pods in the selected namespace"

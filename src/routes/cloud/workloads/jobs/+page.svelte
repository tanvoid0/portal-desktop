...
<script lang="ts">
  ...
  const jobStats = $derived({
    ...
    completed: $cloudStore.resources[ResourceType.JOB].filter(
      (j: any) => j.status === "succeeded" || j.status === "completed",
    ).length,
    running: $cloudStore.resources[ResourceType.JOB].filter(
      (j: any) => j.status === "running",
    ).length,
    failed: $cloudStore.resources[ResourceType.JOB].filter(
  ...
  const resourceActions = useResourceActions({
    ...
    handlers: {
      onDescribe: (resource) => {
        goto(
          `/cloud/workloads/jobs/${resource.name}?namespace=${resource.namespace}`,
        );
      },
      onEdit: (resource) => {
        goto(
          `/cloud/workloads/jobs/${resource.name}?namespace=${resource.namespace}&tab=yaml`,
        );
      },
      onYaml: (resource) => {
        goto(

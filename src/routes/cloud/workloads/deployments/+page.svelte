  ...
  const resourceActions = useResourceActions({
    ...
    handlers: {
      ...
      onRestart: async (resource) => {
        try {
          await k8sResourceService.rollbackDeployment(resource.namespace, resource.name);
...
<div class="space-y-6 p-6">
  {#if $cloudStore.connection.isConnected}
    ...
    <Card class="p-4">
      ...
      {:else}
        <div
          class="k8s-navigable-table overflow-x-auto"
            ...
            <thead>
              <tr class="border-b">
                {#each deploymentColumns as column}
                  <th class="p-2 text-left font-medium">{column.label}</th>
                {/each}

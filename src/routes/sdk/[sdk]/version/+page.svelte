...
<script lang="ts">
	...
	async function loadVersions() {
		...
		try {
			...
			const [installedVersionsResult, availableVersionsResult, currentVersionResult] = await Promise.all([
				invokeClient.post<string[]>('get_manager_installed_versions', { managerName: manager.id }).catch(() => [] as string[]),
				invokeClient.post<string[]>('get_manager_available_versions', { managerName: manager.id }).catch(() => [] as string[]),
				invokeClient.post<string | null>('get_manager_current_version', { managerName: manager.id }).catch(() => null as string | null)
			]);
			...
			versions = Array.from(allVersions).map(version => ({
				...
				error: undefined,
				lts: version.toLowerCase().includes('lts') || version.match(/^\d+\.\d+\.\d+$/)?.[0] !== undefined,
	...
	async function uninstallVersion(version: VersionInfo) {
		...
		if (!confirm(`Are you sure you want to uninstall version ${version.version}?`)) {
			return;
		}
			...
			<div class="flex items-center justify-between">
				<div class="flex items-center gap-4">
					...
					<div>
						...
						<p class="text-muted-foreground">{sdkConfig.display_name} Versions</p>
					</div>
				</div>
			</div>
				...
				<Card>
					<CardContent class="pt-6">
						<Alert>
							<AlertCircle class="h-4 w-4" />
							<AlertDescription>
								No SDK manager is installed for this SDK. Install a version manager (e.g., {sdkConfig.sdk_managers.map(m => m.display_name).join(', ')}) to manage versions.
							</AlertDescription>
						</Alert>
					</CardContent>
				</Card>

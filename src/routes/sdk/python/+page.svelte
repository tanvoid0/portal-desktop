<!--
	Python SDK Detail Page - FlyEnv-style tabbed interface
	...
	async function loadData() {
		...
		try {
			...
			configFiles = [
				...
				{
					name: '.python-version',
					path: '/path/to/project/.python-version',
					exists: true,
					lastModified: new Date().toISOString()
				},
				{
					name: 'pyproject.toml',
					path: '/path/to/project/pyproject.toml',
					exists: false,
					lastModified: null
				}
			];
			...
			environmentVars = [
				{ name: 'PYTHONPATH', value: '/usr/local/lib/python3.11/site-packages', scope: 'global' },
				{ name: 'VIRTUAL_ENV', value: '/path/to/venv', scope: 'project' }
			];
			
		} catch (err) {
			error = err instanceof Error ? err.message : 'Failed to load Python data';
			console.error('Failed to load Python data:', err);
		} finally {
			loading = false;
		}
	}
...
<svelte:head>
	<title>Python SDK - Portal Desktop</title>
</svelte:head>
		...
		<div class="flex items-center gap-4">
			...
			<div>
				...
				<p class="text-muted-foreground">
					Manage Python versions, virtual environments, and project configurations
				</p>
			</div>
		</div>
					...
					<div class="space-y-4">
						...
						{:else}
							<div class="text-center py-8 text-muted-foreground">
								...
								<p>No Python services running</p>

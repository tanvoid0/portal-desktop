...
<script lang="ts">
  ...
  const activeTab = $derived(resolveUrlTab($page.url.searchParams, MANAGER_TABS, 'installed'));
  ...
  async function loadVersions() {
    ...
    try {
      const [installed, available, current] = await Promise.all([
        invoke<string[]>('get_manager_installed_versions', {
          managerName: manager.id,
        }).catch(() => []),
        invoke<string[]>('get_manager_available_versions', {
          managerName: manager.id,
        }).catch(() => []),
        invoke<string | null>('get_manager_current_version', {
          managerName: manager.id,
        }).catch(() => null),
      ]);
...
<div class="space-y-6">
  ...
  {:else if error}
    <PageError title="Failed to load SDK manager" message={error} onRetry={loadManagerDetails} />
  {:else if manager}
    <div class="space-y-6">
      <!-- Header -->
          ...
          <CardContent class="pt-6">
            ...
            {#if manager?.website}
              <div class="mt-4">
                <Button
                  variant="outline"
                  onclick={() => manager?.website && window.open(manager.website, '_blank')}
                >
      ...
      {:else}
          ...
          <Card>
            ...
            <CardContent>
              <div class="flex items-center justify-between">
                ...
                <Badge variant="default" class="bg-blue-100 text-blue-800">Active</Badge>
              </div>
            </CardContent>
          </Card>
        {/if}
        ...
        <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as ManagerTab)}>
          <TabsList>
            ...
            <Card>
              ...
              <CardContent>
                ...
                {:else if installedVersions.length === 0}
                  <p class="py-8 text-center text-muted-foreground">No versions installed</p>
                {:else}
                  <div class="space-y-2">
                      ...
                      >
                        <div class="flex items-center gap-3">
                          ...
                          {#if currentVersion === version}
                            <Badge variant="default" class="bg-blue-100 text-xs text-blue-800">
                              Current
                            </Badge>
                          {/if}
                        </div>
            ...
            <Card>
              ...
              <CardContent>
                ...
                {:else if availableVersions.length === 0}
                  <p class="py-8 text-center text-muted-foreground">No versions available</p>
                {:else}

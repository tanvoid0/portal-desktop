...
<script lang="ts">
  ...
  async function loadManagerDetails() {
    ...
    try {
      ...
      const foundManager = allManagers.find(
        (m) => m.id === managerName || m.name === managerName,
      );
      ...
      error =
        err instanceof Error ? err.message : "Failed to load manager details";
...
{:else if error}
  <PageError
    title="Failed to load SDK manager"
    message={error}
      ...
      <div class="flex flex-1 items-center gap-3">
        ...
        <div>
          ...
          <p class="text-muted-foreground">
            {manager.id} - {manager.supports_installation
      ...
      <Card>
        <CardContent class="pt-6">
          <p class="mb-4 text-muted-foreground">
            This SDK manager is not installed. Install it to manage SDK
            versions.
          </p>
          ...
          {#if manager?.website}
            <div class="mt-4">
              <Button
                variant="outline"
                onclick={() =>
                  manager?.website && window.open(manager.website, "_blank")}
              >

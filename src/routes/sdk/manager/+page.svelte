...
<script lang="ts">
	...
	import { CheckCircle, XCircle, Download, ExternalLink, RefreshCw, ArrowRight } from '@lucide/svelte';
...
<div class="space-y-6 p-6 w-full max-w-none">
	...
	{:else if managers.length === 0}
		<div class="text-center py-12">
			<p class="text-muted-foreground">No SDK managers found</p>
		</div>
	{:else}
		<!-- Managers Grid -->
				...
				<Card class="relative">
					<CardHeader>
						<div class="flex items-center justify-between">
							<button
								type="button"
								class="flex items-center gap-3 flex-1 cursor-pointer hover:opacity-80 text-left"
							...
							>
								...
								<div>
									...
									{#if manager.version}
										<p class="text-xs text-muted-foreground mt-1">{manager.version}</p>
									{/if}
								</div>
							</button>
							<div class="flex items-center gap-2">
					...
					<CardContent>
							...
							{:else if manager.supports_installation && manager.install_command}
								...
								>
									{#if installingManager === manager.id}
										<Download class="w-4 h-4 mr-1 animate-pulse" />
										Installing...
									{:else}
						...
						{#if manager.install_command && !manager.installed}
							<div class="mt-4">
								<p class="text-xs font-medium text-muted-foreground mb-1">Installation Command:</p>

...
<script lang="ts">
  ...
  import {
    avatarStore,
    avatarActions,
  } from "$lib/domains/ai/stores/avatarStore";
  ...
  import type {
    AvatarState,
    AvatarExpression,
  } from "$lib/domains/ai/types/avatar";
  ...
  function getBottomRightPosition() {
    if (typeof window === "undefined") {
      return {
        x: 1000 - AVATAR_SIZE - PADDING,
        y: 800 - AVATAR_SIZE - PADDING,
      };
    }
  ...
  const currentExpression = $derived(
    expressions[avatarExpression] || expressions.neutral,
  );
...
{#if avatarEnabled && isVisible}
  ...
  >
    <!-- Avatar SVG -->
    <div class="relative">
      <svg width="64" height="64" viewBox="0 0 50 50" class="drop-shadow-lg">
        <!-- Head circle -->
        <circle
          cx="25"
          cy="25"

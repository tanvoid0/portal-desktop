<script lang="ts">
  ...
  interface Props {
    /** When true, omits page-level heading (for embedding in settings). */
    embedded?: boolean;
  }
  ...
  function handleButtonClick() {
    toast.success("Button clicked — UI components are working correctly.");
  }
</script>

<div class={embedded ? "space-y-6" : "container mx-auto space-y-8 p-6"}>
  {#if !embedded}
      ...
      <div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
        ...
        <Card>
          ...
          <CardContent class="space-y-3">
            ...
            <div class="flex flex-wrap gap-2">
              <Badge class="bg-status-success-bg text-status-success"
                >Success</Badge
              >
              <Badge class="bg-status-warning-bg text-status-warning"
                >Warning</Badge
              >
              <Badge class="bg-primary/10 text-primary">Info</Badge>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent class="space-y-4">
            ...
            <div class="space-y-4">
              ...
              <div class="flex items-center space-x-2">
                ...
                <Label
                  >Checkbox: {checkboxValue ? "Checked" : "Unchecked"}</Label
                >
              </div>
            </div>
          </CardContent>
        ...
        <Card>
          ...
          <CardContent>
            <div class="grid grid-cols-4 gap-4">
              <div
                class="flex flex-col items-center space-y-2 rounded-lg bg-muted p-3"
              >
                <User class="h-6 w-6" />
        ...
        <Card>
          ...
          <CardContent class="space-y-4">
            <Alert>
              <Info class="h-4 w-4" />
              <AlertDescription
                >This is an informational alert message.</AlertDescription
              >
            </Alert>
            ...
            <Alert variant="destructive">
              ...
              <AlertDescription
                >This is a destructive alert message.</AlertDescription
              >
            </Alert>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div class="grid grid-cols-2 gap-3">
              <div
                class="rounded-lg bg-primary p-3 text-center text-primary-foreground"
              >
                <div class="font-semibold">Primary</div>
                <div class="text-xs opacity-90">hsl(var(--primary))</div>
              </div>
              <div
                class="rounded-lg bg-secondary p-3 text-center text-secondary-foreground"
              >
                <div class="font-semibold">Secondary</div>
                <div class="text-xs opacity-90">hsl(var(--secondary))</div>
              </div>
              <div
                class="rounded-lg bg-muted p-3 text-center text-muted-foreground"
              >
                <div class="font-semibold">Muted</div>
                <div class="text-xs opacity-90">hsl(var(--muted))</div>
              </div>
              <div
                class="rounded-lg bg-accent p-3 text-center text-accent-foreground"
              >
  ...
  {#if !embedded}
    <Card class="mt-8">
      <CardHeader>
        ...
        <CardDescription
          >Verification that all UI components are working correctly</CardDescription
        >
      </CardHeader>
      <CardContent>
        <div class="grid grid-cols-2 gap-4 md:grid-cols-4">
          {#each ["Buttons", "Cards", "Forms", "Icons"] as label}
            <div
              class="flex items-center space-x-2 rounded-lg bg-status-success/10 p-3"
            >

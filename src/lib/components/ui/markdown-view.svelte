  ...
  function markdownToHtml(text: string): string {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (trimmed.match(/^### /)) {
        flushList();
        processed.push(
          `<h3 class="text-lg font-semibold mt-4 mb-2">${escapeHtml(trimmed.substring(4))}</h3>`,
        );
        continue;
      }
      if (trimmed.match(/^## /)) {
        flushList();
        processed.push(
          `<h2 class="text-xl font-semibold mt-5 mb-3">${escapeHtml(trimmed.substring(3))}</h2>`,
        );
        continue;
      }
      if (trimmed.match(/^# /)) {
        flushList();
        processed.push(
          `<h1 class="text-2xl font-bold mt-6 mb-4">${escapeHtml(trimmed.substring(2))}</h1>`,
        );
        continue;
      }
      ...
      if (trimmed.match(/^[\*\-+] /)) {
        ...
        listItems.push(
          `<li class="ml-4">${escapeHtml(trimmed.substring(2))}</li>`,
        );
        continue;
      }
      ...
      if (trimmed.match(/^> /)) {
        flushList();
        processed.push(
          `<blockquote class="border-l-4 border-muted-foreground/30 pl-4 my-2 italic">${escapeHtml(trimmed.substring(2))}</blockquote>`,
        );
        continue;
      }
      ...
      if (trimmed) {
        processed.push(
          `<p class="mb-3 leading-relaxed">${escapeHtml(trimmed)}</p>`,
        );
      } else {
        processed.push("");
      }
    ...
    codeBlockPlaceholders.forEach((code, index) => {
      html = html.replace(
        ...
        `<pre class="bg-muted p-3 rounded-md overflow-x-auto my-2"><code>${code}</code></pre>`,
      );
    });

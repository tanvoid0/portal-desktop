...
<script lang="ts">
  ...
  let { icon, iconType, size = 'md', class: className = '', alt = '' }: Props = $props();
  ...
  const isDevicon = $derived(
    iconType === 'devicon' || icon.startsWith('devicon-') || icon.startsWith('devicon ')
  );
  const deviconClass = $derived(icon.startsWith('devicon-') ? `devicon ${icon}` : icon);
</script>

	...
	async function pollForApproval(maxAttempts: number = 60) {
		...
		for (let i = 0; i < maxAttempts; i++) {
			...
			try {
				...
				if (status?.message && (status.message.includes('rejected') || status.message.includes('denied'))) {
					pollingStatus = null;
		...
		pollingStatus = null;
		error = 'Approval timeout. The host device may not have approved your request. Please request a new passcode.';
		loading = false;
	}
...
<div class="flex items-center justify-center min-h-screen bg-background p-4">
	<Card class="w-full max-w-md">
		<CardHeader class="text-center">
			...
			<CardDescription>
				This application requires device approval for browser access.
				Please enter the 6-digit passcode provided by the host device.
			</CardDescription>
		</CardHeader>
		<CardContent class="space-y-4">
			...
			{:else}
				<div class="space-y-4">
					...
					<div>
						<label for="passcode" class="text-sm font-medium mb-2 block">
							Enter Passcode
						</label>
						<Input
							id="passcode"
					...
					<div class="text-xs text-muted-foreground space-y-1 pt-2 border-t">
						...
						<p><strong>Device ID:</strong> <code class="text-xs">{deviceInfo.device_id}</code></p>
					</div>
				</div>
			{/if}
		</CardContent>

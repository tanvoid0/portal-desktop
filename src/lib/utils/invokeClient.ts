...
export class InvokeClient {
	...
	async live<T = any>(
		...
		const uniqueStreamId = streamId || `stream-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
	...
	private async browserWebSocket<T>(
		...
		const wsUrl = baseUrl.replace(/^http/, 'ws') + `/ws/${endpoint}?streamId=${encodeURIComponent(streamId)}`;

...
class LearningService {
  ...
  private scheduleAutomaticCleanup(): void {
    ...
    this.cleanupInterval = setInterval(async () => {
      try {
        ...
        const totalToDelete =
          preview.events_to_delete + preview.patterns_to_delete;
  ...
  async getSuggestions(
    ...
    try {
      ...
      const cached = cache.getList<Suggestion>(cacheKey);
      if (cached !== null) {
      ...
      cache.setList(cacheKey, suggestions ?? [], 2 * 60 * 1000);
   ...
   */
  async markPatternImportant(
    patternId: number,
    isImportant: boolean,
  ): Promise<void> {
    try {
      log.info("Marking pattern as important", {
      ...
      log.info("Pattern importance updated successfully", {
        pattern_id: patternId,
      });
    } catch (error) {
  ...
  async getPreference(
    ...
    try {
      log.info("Getting preference", {
        preference_type: preferenceType,
        context,
      });

      const preference = await invoke<Record<string, unknown> | null>(
        "get_preference",
        {
          preferenceType,
          context: context ?? null,
  ...
  async getAllPatterns(limit?: number): Promise<LearnedPattern[]> {
    try {
      ...
      const patterns = await invoke<LearnedPattern[]>("get_all_patterns", {
        limit: limit ?? null,
      });
      return patterns;
    } catch (error) {
  ...
  async getRecentEvents(limit: number = 50): Promise<LearningEvent[]> {
    try {
      ...
      const events = await invoke<LearningEvent[]>("get_recent_events", {
        limit,
      });
      return events;
    } catch (error) {

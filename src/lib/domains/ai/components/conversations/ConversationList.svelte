<script lang="ts">
  ...
  import { Plus, MessageCircle, Trash2, FolderPlus, ChevronRight, X } from '@lucide/svelte';
  ...
  const isEmpty = $derived(filteredConversations.length === 0 && folders.names.length === 0);
...
>
  ...
  {#snippet empty()}
    <div
      class="flex flex-1 flex-col items-center justify-center py-8 text-center text-muted-foreground"
    >
      ...
      >
        ...
        <div class="space-y-0.5 pl-3">
          ...
          {#if group.items.length === 0}
            <p class="px-2 py-1 text-[11px] text-muted-foreground">Folder is empty</p>
          {/if}
        </div>
      </details>
    {:else}

...
export function sortCatalogProviders(providers: CatalogProvider[]): CatalogProvider[] {
  const order = new Map(CATALOG_PROVIDER_ORDER.map((id, index) => [id, index]));
...
export function selectableCatalogProviders(providers: CatalogProvider[]): CatalogProvider[] {
  return sortCatalogProviders(providers).filter(
    (provider) => provider.configured && provider.reachable !== false && provider.models.length > 0
  );
}

function withProviderId(models: CatalogModel[], providerId: string): CatalogModel[] {
...
export function modelsForCatalogProvider(
  ...
  if (!provider) return [];
  return withProviderId(provider.models, providerId).sort((a, b) => a.id.localeCompare(b.id));
}
...
const MODEL_BADGE_STYLES = {
  params: {
    badge: 'border-sky-200/80 bg-sky-50 dark:border-sky-800 dark:bg-sky-950/60',
    value: 'text-sky-700 dark:text-sky-300',
  },
  family: {
    badge: 'border-violet-200/80 bg-violet-50 dark:border-violet-800 dark:bg-violet-950/60',
    value: 'text-violet-700 dark:text-violet-300',
  },
  quantization: {
    badge: 'border-amber-200/80 bg-amber-50 dark:border-amber-800 dark:bg-amber-950/60',
    value: 'text-amber-800 dark:text-amber-300',
  },
  size: {
    badge: 'border-slate-200/80 bg-slate-50 dark:border-slate-700 dark:bg-slate-900/60',
    value: 'text-slate-600 dark:text-slate-300',
  },
  provider: {
    badge: 'border-indigo-200/80 bg-indigo-50 dark:border-indigo-800 dark:bg-indigo-950/60',
    value: 'text-indigo-700 dark:text-indigo-300',
  },
  live: {
    badge: 'border-emerald-200/80 bg-emerald-50 dark:border-emerald-800 dark:bg-emerald-950/60',
    value: 'text-emerald-700 dark:text-emerald-300',
  },
  alias: {
    badge: 'border-orange-200/80 bg-orange-50 dark:border-orange-800 dark:bg-orange-950/60',
    value: 'text-orange-700 dark:text-orange-300',
  },
  extra: {
    badge: 'border-zinc-200/80 bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-900/60',
  ...
  toolsNo: {
    badge: 'border-rose-200/80 bg-rose-50 dark:border-rose-800 dark:bg-rose-950/60',
    value: 'text-rose-700 dark:text-rose-300',
  },
  vision: {
    badge: 'border-cyan-200/80 bg-cyan-50 dark:border-cyan-800 dark:bg-cyan-950/60',
    value: 'text-cyan-700 dark:text-cyan-300',
  },
} as const;

function capabilityBadges(caps: CatalogModelCapabilities | undefined): ModelDisplayBadge[] {
  if (!caps) return [];
...
export function getModelDisplayParts(model: CatalogModel): ModelDisplayParts {
  ...
  const parameterSize = meta.parameter_size;
  if (typeof parameterSize === 'string' && parameterSize) {
    specBadges.push(makeBadge('params', 'Params', parameterSize, 'spec', 'params'));
  }

  const family = meta.family;
  if (typeof family === 'string' && family) {
    specBadges.push(makeBadge('family', 'Family', family, 'spec', 'family'));
  }

  const quantization = meta.quantization_level;
  if (typeof quantization === 'string' && quantization) {
    specBadges.push(makeBadge('quantization', 'Quant', quantization, 'spec', 'quantization'));
  }

  const size = meta.size;
  if (typeof size === 'number' && size > 0) {
    specBadges.push(makeBadge('size', 'Size', formatBytes(size), 'spec', 'size'));
  }

  specBadges.push(...capabilityBadges(model.capabilities));

  if (model.provider && model.provider !== 'unknown') {
    specBadges.push(makeBadge('provider', 'Provider', model.provider, 'spec', 'provider'));
  }
  ...
  for (const [key, value] of Object.entries(meta)) {
    ...
    specBadges.push(makeBadge(`extra-${key}`, formatMetadataKey(key), formatted, 'spec', 'extra'));
  }

  if (model.source === 'alias') {
    statusBadges.push(makeBadge('alias', 'Source', 'alias', 'status', 'alias'));
  } else if (model.source === 'live') {
    statusBadges.push(makeBadge('live', 'Source', 'live', 'status', 'live'));
  }
  ...
  return {
    id: model.id,
    provider: model.provider && model.provider !== 'unknown' ? model.provider : undefined,
    backendId,
    displayName:
      typeof displayName === 'string' && displayName && displayName !== model.id
        ? displayName
        : undefined,
    specBadges,
    statusBadges,
...
export function getModelRow(model: CatalogModel): ModelRow {
  ...
  return {
    ...
    publisher: model.provider && model.provider !== 'unknown' ? model.provider : undefined,

...
import { toastStore } from '$lib/domains/shared/stores/toastStore';
...
class AvatarService {
	 ...
	 */
	private async handleToast(toast: { id: string; title?: string; description?: string; type?: 'success' | 'error' | 'warning' | 'info' }) {
	 ...
	 */
	private getErrorKey(toast: { title?: string; description?: string; type?: string }): string {
	 ...
	 */
	private async generateSuggestion(toast: { title?: string; description?: string; type?: string }) {
		try {
			// Set thinking state
			...
			// Build prompt
			const errorMessage = toast.description || toast.title || 'An error occurred';
	 ...
	 */
	private getFallbackSuggestion(toast: { title?: string; description?: string; type?: string }): string {
	 ...
	 */
	private showSuggestion(message: string, toast: { title?: string; description?: string; type?: string }) {
		const suggestion: AvatarSuggestion = {
			...
			context: {
				...
				toastType: toast.type as 'error' | 'warning' | 'info' | 'success' | undefined
			}
		};
	 ...
	 */
	async triggerSuggestion(message: string, context?: { error?: string; page?: string }) {
		const suggestion: AvatarSuggestion = {
			id: `suggestion-${Date.now()}`,
			message,
			timestamp: new Date(),

<script lang="ts">
  ...
  const failedOutput = $derived(failed && result ? formatFailedResult(tool, result) : '');
  ...
  {#if open}
    <div class="ml-4 mt-1 space-y-2 border-l border-border/60 pl-3 pb-1">
      ...
      {:else if isListDir && result !== null && !failed}
        <ul
          class="grid max-h-48 grid-cols-2 gap-x-2 gap-y-0.5 overflow-auto text-[11px] sm:grid-cols-3"
        >
      ...
      {:else if failed && result !== null}
        <pre
          class="max-h-48 overflow-auto rounded bg-destructive/10 p-2 text-[11px] text-destructive"><code
            >{failedOutput}</code
          ></pre>

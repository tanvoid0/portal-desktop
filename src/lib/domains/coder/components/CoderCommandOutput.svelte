<script lang="ts">
  ...
  let { threadId, callId, command, output, failed = false, workspaceRoot = '' }: Props = $props();
...
<div
  class="coder-command-output h-48 min-h-0 w-full overflow-hidden rounded border border-border bg-[#0c0c0c]"
>
  <Terminal
    bind:this={terminal}
    {tabId}

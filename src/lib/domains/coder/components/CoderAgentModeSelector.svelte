<script lang="ts">
  ...
  import { AGENT_MODES, autoModeSubtitle, getAgentModeConfig } from '../config/agentModes.js';
  ...
  let { mode, onModeChange, effectiveMode = null, disabled = false }: Props = $props();
...
<DropdownMenu.Root>
  ...
  <DropdownMenu.Content align="end" class="w-56 p-1">
    <DropdownMenu.RadioGroup value={mode} onValueChange={(v) => onModeChange(v as CoderAgentMode)}>
      {#each AGENT_MODES as m (m.value)}
        <DropdownMenu.RadioItem
          value={m.value}
          class={cn('gap-2 rounded-md py-1.5 pl-7 pr-2 text-xs', m.itemClass)}
        >

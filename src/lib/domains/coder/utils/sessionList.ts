...
export function filterAndSortSessions(
  ...
  if (filters.provider !== 'all') {
    result = result.filter((t) => (t.llm_provider ?? t.model ?? '') === filters.provider);
  }

  if (filters.status !== 'all') {
    result = result.filter((t) => {
      const queued = queuedCountFor?.(t.id) ?? 0;
      return sessionStatus(t.id, runningThreadIds, queued) === filters.status;
    });
  }

  result.sort((a, b) => compareSessions(a, b, filters.sort, runningThreadIds));
  return result;
}
...
export function sortThreadsForWorkspace(
  ...
  return [...threads].sort((a, b) => compareSessions(a, b, sort, runningThreadIds));
}

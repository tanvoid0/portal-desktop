...
export async function getDashboardOverview(opts?: { force?: boolean }): Promise<DashboardOverview> {
	const force = opts?.force ?? false;

	// Fast path: return cached (TTL-aware)
	if (!force) {
		const cached = cache.get<DashboardOverview>(DASHBOARD_OVERVIEW_CACHE_KEY);
		if (cached) return cached;
	}

	// Fetch from backend (lightweight endpoint)
	try {
		const overview = await invokeClient.post<DashboardOverview>('get_dashboard_overview');
		return overview ?? EMPTY_OVERVIEW;
	} catch {
		// For browser/localhost strategies where Tauri commands are unavailable, return safe defaults
...
export function primeDashboardOverview(overview: DashboardOverview, ttlMs: number = DEFAULT_TTL_MS): void {

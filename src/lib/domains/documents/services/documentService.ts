...
function convertTauriDocumentToDocument(
  tauriDoc: TauriDocumentResponse,
): Document {
  return {
    id: tauriDoc.id,
    ...
    lastEditedAt: tauriDoc.last_edited_at
      ? new Date(tauriDoc.last_edited_at)
      : undefined,
  };
}
...
export class DocumentService {
  async createDocument(request: CreateDocumentRequest): Promise<Document> {
    ...
    const response = await invoke<TauriDocumentResponse>("create_document", {
      command,
    });
  ...
  async updateDraft(id: number, contentDraft: string): Promise<Document> {
    const response = await invoke<TauriDocumentResponse>(
  ...
  async getDocument(id: number): Promise<Document | null> {
    const response = await invoke<TauriDocumentResponse | null>(
      "get_document",
      { id },
    );
  ...
  async searchDocuments(query: string): Promise<Document[]> {
    const response = await invoke<TauriDocumentResponse[]>("search_documents", {
      query,
    });
    return response.map(convertTauriDocumentToDocument);
  }

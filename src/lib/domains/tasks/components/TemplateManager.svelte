...
<Card class="w-full">
  ...
  <CardContent class="p-4 pt-0">
    {#if taskUi.taskTemplates.length === 0}
      <div class="py-8 text-center text-muted-foreground">
        <Icon icon="mdi:template" class="mx-auto mb-2 h-12 w-12" />
        <div class="text-sm">No templates yet</div>
        <div class="text-xs">Create your first template to get started</div>
      </div>
    {:else}
      <div class="space-y-2">
        {#each taskUi.taskTemplates as template}
          <div
          ...
          >
            <div class="flex items-center gap-3">
              <Button
                variant="ghost"
                class="h-auto flex-1 justify-start p-0"
                onclick={() => applyTemplate(template)}
              >
                <div class="flex items-center gap-2">
                  <Icon icon="mdi:template" class="h-4 w-4" />
                  <div class="text-left">
                    ...
                    <div class="mt-1 flex items-center gap-2">
                      ...
                      {#if template.defaultEstimatedTime}
                        <Badge variant="outline" class="text-xs"
                          >{template.defaultEstimatedTime}m</Badge
                        >
                      {/if}
                    </div>
                  </div>

...
<script lang="ts">
  ...
  async function handleStartDocker() {
    try {
      ...
      toast.error(error instanceof Error ? error.message : 'Failed to start Docker');
    }
  }
  ...
  async function handleRetry() {
    try {
      ...
      toast.error(error instanceof Error ? error.message : 'Failed to check Docker status');
    }
  }
</script>
    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      <div class="space-y-1">
        <AlertTitle class="flex items-center gap-2">
          Docker
          {#if status.running}
            <Badge variant="default" class="bg-green-100 text-green-800">Running</Badge>
            {#if status.version}
        ...
        <AlertDescription>
          ...
          {:else}
            {status.message ?? 'Docker is not running. Start Docker Desktop to manage containers.'}
          {/if}
        </AlertDescription>
      </div>
      ...
      {#if !status.running}
        <div class="flex shrink-0 gap-2">
          {#if status.installed}
            <Button size="sm" onclick={handleStartDocker} disabled={starting}>
              {#if starting}

...
function summarizeExecutionFailure(exec: ScriptExecutionInfo): string {
  ...
  if (tail) {
    return exec.exitCode != null ? `Exit code ${exec.exitCode}\n${tail}` : tail;
  }
...
export interface ProjectActionsHandle {
  ...
  run: (target: string | string[], options?: ActionRunOptions) => Promise<ActionRunResult>;
  preview: (target: string | string[], options?: ActionRunOptions) => Promise<ActionPlanPreview>;
}

function topoSortSteps(steps: UnifiedWorkflow['steps'], only?: string[]): string[] {
  const filtered = only?.length ? steps.filter((s) => only.includes(s.action)) : steps;
  const ids = new Set(filtered.map((s) => s.action));
  const needs = new Map(filtered.map((s) => [s.action, (s.needs ?? []).filter((n) => ids.has(n))]));
  const result: string[] = [];
...
class ActionsService {
  ...
  async catalogForProject(
    ...
    return {
      ...
      warnings: [...local.warnings, ...github.warnings, ...n8n.warnings],
    };
  }
  ...
  forProject(project: Project): ProjectActionsHandle {
    return {
      list: async () => (await this.catalogForProject(project)).actions,
      workflows: async () => (await this.catalogForProject(project)).workflows,
      catalog: () => this.catalogForProject(project),
      run: (target, options) => this.run(target, { ...options, project }),
      preview: (target, options) => this.preview(target, { ...options, project }),
    };
  }
  ...
  async run(
    ...
    const cwd =
      options.cwd ?? options.project?.path ?? (typeof target === 'string' ? undefined : undefined);
  ...
  private async runActionList(
    ...
    for (const action of actions) {
      ...
      if (action.runner === 'n8n') {
        const remote = await runN8nAction(action.webhookId ?? action.id, options.project);
        results.push({
          name: action.name,
          actionId: action.id,
          status: remote.success ? 'success' : 'failed',
      ...
      try {
        ...
        const finished = await this.waitForExecution(executionId, action.longRunning);
        ...
        results.push({
          ...
          error: stepSuccess ? undefined : summarizeExecutionFailure(finished),
          runner: 'local',
        });
        if (!stepSuccess) {
          success = false;
...
export const actions = {
  ...
  forDirectory: (cwd: string, packageManager?: string) => ({
    list: async () => (await actionsService.catalogForDirectory(cwd, { packageManager })).actions,
    workflows: async () =>
      (await actionsService.catalogForDirectory(cwd, { packageManager })).workflows,
    catalog: () => actionsService.catalogForDirectory(cwd, { packageManager }),

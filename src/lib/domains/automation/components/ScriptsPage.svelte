...
<script lang="ts">
  ...
  setBreadcrumbs([
    ...
    { label: 'Scripts', href: '/automation/scripts' },
  ]);
...
<div class="container mx-auto p-6 space-y-6">
  <PageHeader
    title="Scripts"
    description="Create, manage, and run reusable scripts for your workflows"
  >
    {#snippet actions()}
      ...
      {#each filteredScripts as script (script.id)}
        ...
        >
          ...
          <CardContent class="space-y-4">
            <!-- Command Preview -->
            <div
              class="bg-muted rounded-md p-2 relative group"
              onclick={(e) => e.stopPropagation()}
            >
              <code class="text-xs font-mono text-muted-foreground line-clamp-2"
                >{script.command}</code
              >
              <Button
                variant="ghost"
                ...
                onclick={(e) => {
                  e.stopPropagation();
                  handleCopyCommand(script.command);
                }}
              >
                <Copy class="h-3 w-3" />
              </Button>
            ...
            <!-- Actions -->
            <div
              class="divider-edge-t divider-edge-full flex gap-2 pt-2"
              onclick={(e) => e.stopPropagation()}
            >
              <Button
                size="sm"

...
const DETAIL_SKIP = /^(?:\+|At line:|CategoryInfo\s*:|FullyQualifiedErrorId\s*:|~~+)/i;
...
export function parseTerminalError(output: string, command?: string): ParsedTerminalError | null {
  ...
  // syntax
  if (/ParserError|syntax error|Unexpected token|Parse error|invalid syntax/i.test(raw)) {
    return {
  ...
  return {
    ...
    message: details.length > 1 ? details.slice(1, 4).join('\n') : title,

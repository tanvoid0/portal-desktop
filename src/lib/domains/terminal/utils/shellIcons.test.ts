...
describe('resolveShellIcon', () => {
  ...
  it('maps profile names and container commands', () => {
    ...
    expect(resolveShellIcon('docker exec -it abc bash')).toBe(TERMINAL_ICONS.container);
...
describe('resolveTabIcon', () => {
  ...
  it('falls back to shell and tab type', () => {
    ...
    expect(resolveTabIcon({ type: 'editor' })).toBe('mdi:file-document-edit-outline');
    expect(resolveTabIcon({ resourceName: 'container' })).toBe(TERMINAL_ICONS.container);
  });
});

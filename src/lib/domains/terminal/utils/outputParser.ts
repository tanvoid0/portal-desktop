...
function cleanRawOutput(text: string): string {
  ...
  cleaned = cleaned.replace(/[\u001b\u009b][[()#;?]*(?:[0-9]{1,4}(?:;[0-9]{0,4})*)?[0-9A-PRZcf-nqry=><]/g, '');
...
function extractOutputBetweenPrompts(
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (!foundCommandStart) {
      ...
      const hasCommand = trimmed.includes(commandTrimmed) || 
    ...
    if (foundCommandStart) {
      ...
      if (firstCommandWord && trimmed.length > 0) {
        ...
        const isCommandArtifact = words.every(word => 
          word.length > 0 && (
  ...
  if (outputLines.length === 0) {
    ...
    for (let i = lines.length - 1; i >= 0; i--) {
      ...
      if (lineTrimmed.includes(commandTrimmed) || 
  ...
  if (outputLines.length === 0) {
    ...
    for (const line of lines) {
      ...
      const isPrompt = promptPattern.test(trimmed) ||
                      /^[%~$#❯>]+$/.test(trimmed) ||
                      /~\s*❯/.test(trimmed) ||
                      /\d{2}:\d{2}:\d{2}\s*(AM|PM)/.test(trimmed);
      
      const isCommand = trimmed.includes(commandTrimmed) ||
                       trimmed.includes(firstCommandWord);
  ...
  if (outputLines.length > 0) {
    ...
    for (const line of outputLines) {
      ...
      // Skip timestamp lines
      if (/\d{2}:\d{2}:\d{2}\s*(AM|PM)/.test(trimmed) || /^[A-Z][a-z]{2}\s+\d{1,2}\s+\d{2}:\d{2}:\d{2}/.test(trimmed)) {
        continue;
      }

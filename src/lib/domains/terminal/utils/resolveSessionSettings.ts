...
const INTEGRATION_FAMILIES = new Set(['powershell', 'pwsh', 'zsh', 'bash']);
...
export function resolveSessionSettings(
  ...
  if (!shellSupportsIntegration(shell) && shellSupportsIntegration(workspaceShell)) {
    shell = workspaceShell;
  }

  const workingDirectory = tab.workingDirectory?.trim() || workspaceSettings.workingDirectory;
...
export function projectTabNeedsMigration(
  ...
  const shellStale =
    !shellSupportsIntegration(tab.shell) && shellSupportsIntegration(preferredShell);
  const cwdStale = !!projectPath && !!tab.workingDirectory && tab.workingDirectory !== projectPath;

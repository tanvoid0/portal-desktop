...
import { subscribeTerminalOutput, ensureTerminalOutputListener } from './terminalOutputBus';
...
export class TerminalService {
  ...
  static async getProcess(processId: string): Promise<TerminalProcess | null> {
    try {
      return await invokeClient.request<TerminalProcess | null>('get_terminal_process', {
        data: { processId },
      });
    } catch (error) {
  ...
  static async executeInContext(
    ...
    try {
      ...
      if (options?.captureBlock !== false) {
        const { commandBlockStore } = await import('../stores/commandBlockStore');
  ...
  private static async handleProcessExit(processId: string): Promise<void> {
    try {
      ...
      const exitCode = await invokeClient.request<number | null>('get_process_exit_code', {
        data: { processId },
      });
  ...
  static async completeCurrentCommand(exitCode?: number, tabId?: string): Promise<void> {
    ...
    if (this.currentCommand) {
      ...
      try {
        ...
        await patternCollector.collectCommandPattern(this.currentCommand.command, success, context);
      } catch (error) {
  ...
  static async getProcessExitCode(processId: string): Promise<number | null> {
    try {
      return await invokeClient.request<number | null>('get_process_exit_code', {
        data: { processId },
      });
    } catch {
      return null;

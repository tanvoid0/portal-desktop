...
describe('resolveShellMetadata', () => {
  it('detects PowerShell 7', () => {
    expect(resolveShellMetadata('C:\\Program Files\\PowerShell\\7\\pwsh.exe').family).toBe('pwsh');
  });

  it('detects Windows PowerShell', () => {
    expect(
      resolveShellMetadata('C:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe').family
    ).toBe('powershell');
  });
...
describe('parseAiResponse', () => {
  ...
  it('handles responses without code blocks', () => {
    expect(parseAiResponse('plain answer')).toEqual([{ type: 'text', content: 'plain answer' }]);
  });

  it('handles unlabeled fences and multiple blocks', () => {
    const segments = parseAiResponse('```\nls\n```\n```\npwd\n```');
    expect(segments.filter((s) => s.type === 'code').map((s) => s.content)).toEqual(['ls', 'pwd']);
  });
});

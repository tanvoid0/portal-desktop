...
import type { BaseItem, BaseSuggestedItem, BaseItemGroup, ItemService } from './types';
import type { Language, LanguageGroup, SuggestedLanguage } from '@/lib/domains/languages/types';
import type { PackageManager, PackageManagerGroup, SuggestedPackageManager } from '@/lib/domains/package_managers/types';
import type { Framework, FrameworkGroup, SuggestedFramework } from '@/lib/domains/ide/services/ideService';
 ...
 */
function normalizeLanguageGroup(group: LanguageGroup): BaseItemGroup<SuggestedLanguage> {
	return {
 ...
 */
function normalizePackageManagerGroup(group: PackageManagerGroup): BaseItemGroup<SuggestedPackageManager> {
	return {
		category: group.category,
 ...
 */
function normalizeFrameworkGroup(group: FrameworkGroup): BaseItemGroup<SuggestedFramework> {
	return {
		category: group.category,
...
export const languageServiceAdapter: ItemService<Language, SuggestedLanguage> = {
	...
	async create(name, icon, iconType, category) {
		return await languageService.createLanguage(name, icon, iconType, category);
	},
	async update(id, name, icon, iconType, category) {
		return await languageService.updateLanguage(id, name, icon, iconType, category);
	},
	async delete(id) {
	...
	async createBatch(items) {
		...
		return {
			...
			failed: result.failed.map(f => ({ item: f.language, error: f.error }))
		};
	}
};
 ...
 */
export const packageManagerServiceAdapter: ItemService<PackageManager, SuggestedPackageManager> = {
	async getAll() {
	...
	async create(name, icon, iconType, category) {
		return await packageManagerService.createPackageManager(name, icon, iconType, category);
	},
	async update(id, name, icon, iconType, category) {
		return await packageManagerService.updatePackageManager(id, name, icon, iconType, category);
	},
	async delete(id) {
		return await packageManagerService.deletePackageManager(id);
	},
	async createBatch(items) {
		const result = await packageManagerService.createPackageManagersBatch(items);
		return {
			success: result.success,
			failed: result.failed.map(f => ({ item: f.packageManager, error: f.error }))
		};
	}
};
 ...
 */
export const frameworkServiceAdapter: ItemService<Framework, SuggestedFramework> = {
	async getAll() {

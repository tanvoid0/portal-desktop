...
<script lang="ts" generics="T extends BaseItem, TSuggested extends BaseSuggestedItem">
	...
	import type { BaseItem, BaseSuggestedItem, BaseItemGroup, ItemService, SettingsComponentConfig } from './types';
	...
	async function loadSuggestedItems() {
		try {
			...
			log.info(`Suggested ${config.itemNamePlural.toLowerCase()} loaded`, { count: suggestedGroups.length });
		} catch (error) {
	...
	async function loadRecommendedItems() {
		try {
			const suggestions = await suggestionEngine.getContextualSuggestions(config.recommendationPatternType);
			...
			log.info(`Recommended ${config.itemNamePlural.toLowerCase()} loaded`, { count: recommendedNames.length });
		} catch (error) {
	...
	async function saveItem() {
		...
		try {
			...
			toast.error(error instanceof Error ? error.message : `Failed to save ${config.itemName.toLowerCase()}`);
		}
	}

	async function deleteItem(id: number, name: string) {
		if (!confirm(`Are you sure you want to delete the ${config.itemName.toLowerCase()} "${name}"?`)) {
			return;
		}
	...
	const mergedSuggestedGroups = $derived.by(() => {
		...
		if (recommendedNames.length > 0) {
			const recommendedItems: TSuggested[] = recommendedNames.map(name => {
				const existing = allSuggestedItems.find(si =>
					si.name.toLowerCase() === name.toLowerCase()
				);
	...
	async function addAllItemsFromGroup(group: BaseItemGroup<TSuggested>) {
		...
		try {
			...
			if (result.success.length > 0) {
				toast.success(`Successfully added ${result.success.length} ${config.itemName.toLowerCase()}${result.success.length > 1 ? 's' : ''}`);
			...
			if (result.failed.length > 0) {
				...
				toast.error(`Failed to add ${result.failed.length} ${config.itemName.toLowerCase()}${result.failed.length > 1 ? 's' : ''}: ${failedNames}`);
			}
...
<div class="space-y-6">
	...
	{#if isLoading}
		<div class="flex items-center justify-center py-12">
			...
			<span class="ml-2 text-muted-foreground">Loading {config.itemNamePlural.toLowerCase()}...</span>
		</div>
	{:else}
				...
				{#if filtered.length === 0}
					<Card>
						<CardContent class="flex flex-col items-center justify-center py-12">
							<svelte:component this={config.emptyIcon} class="w-12 h-12 text-muted-foreground mb-4" />
							...
							<Card class="relative">
								...
								<CardContent>
									<div class="flex items-center justify-end gap-2 mt-2">
										...
										<Button variant="outline" size="sm" onclick={() => deleteItem(item.id, item.name)} class="text-red-500 hover:text-red-700">
								...
								<div class="flex items-center justify-between">
									<div class="flex items-center gap-2">
										...
										<div>
											...
											<CardDescription>
												{group.items.length} {config.itemName.toLowerCase()}{group.items.length > 1 ? 's' : ''} available
											</CardDescription>
										</div>
									</div>
									<Button
	...
	<DialogContent class="sm:max-w-[500px]">
		...
		<div class="grid gap-4 py-4">
			...
			<div class="space-y-2">
				...
				<Input id="item-icon" bind:value={itemIcon} placeholder="logos:example (for Devicon) or image URL" />
				<p class="text-xs text-muted-foreground">Enter a Devicon name (e.g., "logos:javascript") or an image URL</p>
			</div>
			<div class="space-y-2">
				<Label for="item-icon-type">Icon Type</Label>
				<select id="item-icon-type" bind:value={itemIconType} class="w-full px-3 py-2 border rounded-md">
		...
		<DialogFooter>
			...
			<Button onclick={saveItem}>
				{editingItem ? 'Update' : 'Add'} {config.itemName}
			</Button>
		</DialogFooter>
	</DialogContent>
</Dialog>

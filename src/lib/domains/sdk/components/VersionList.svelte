...
<script lang="ts">
  ...
  let availableVersions = $derived(versions.filter((v) => !v.installed));

  let activeVersion = $derived(versions.find((v) => v.active));
    ...
    >
      ...
      <CardContent>
        <div class="flex items-center justify-between">
          ...
          <Badge variant="default" class="bg-green-600">Currently Active</Badge>
        </div>
      </CardContent>
    </Card>
  {/if}

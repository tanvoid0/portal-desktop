...
export function useResourceActions(options: UseResourceActionsOptions) {
  ...
  const selectedIndexStore: Readable<number> =
    typeof selectedIndex === "object" && "subscribe" in selectedIndex
      ? selectedIndex
      : {
  ...
  const resourcesStore: Readable<ICloudResource[]> =
    ...
    resources !== null &&
    "subscribe" in resources
      ? (resources as Readable<ICloudResource[]>)
      : typeof resources === "function"
        ? {
            subscribe: (fn: (val: ICloudResource[]) => void) => {
              fn((resources as () => ICloudResource[])());
              return () => {};
            },
          }
        : {
  ...
  const actions: Readable<ResourceAction[]> = derived(
    ...
    ([$currentResource]) => {
      ...
      if (
        handlers.onLogs &&
        (res.type === "pod" || res.type === "deployment")
      ) {
        availableActions.push({
      ...
      if (
        handlers.onRestart &&
        (res.type === "deployment" || res.type === "statefulset")
      ) {
        availableActions.push({
  ...
  function handleKeydown(event: KeyboardEvent): boolean {
    ...
    if (
      ...
      event.key === ACTION_SHORTCUTS.DELETE
    ) {
      const deleteAction = currentActions.find(
        (a: ResourceAction) => a.key === ACTION_SHORTCUTS.DELETE,
      );
      if (deleteAction && res) {
    ...
    if (!event.ctrlKey && !event.metaKey && !event.altKey && !event.shiftKey) {
      const action = currentActions.find(
        (a: ResourceAction) => a.shortcut.toLowerCase() === key,
      );

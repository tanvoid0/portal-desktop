...
export class PackageManagerService {
	...
	async getAllPackageManagers(): Promise<PackageManager[]> {
		try {
			...
			this.log.info('Package managers retrieved', { count: safePackageManagers.length });
	...
	async getSuggestedPackageManagers(): Promise<PackageManagerGroup[]> {
		try {
			...
			const groups = await invokeClient.post<PackageManagerGroup[]>('get_suggested_package_managers');
			const safeGroups = groups ?? [];
			this.log.info('Suggested package managers retrieved', { count: safeGroups.length });
			return safeGroups;
		} catch (error) {
	...
	async createPackageManager(
		...
		try {
			...
			this.log.info('Package manager created successfully', { id: packageManager.id });
	 ...
	 */
	async createPackageManagersBatch(packageManagers: SuggestedPackageManager[]): Promise<{ success: PackageManager[]; failed: { packageManager: SuggestedPackageManager; error: string }[] }> {
		...
		for (const pm of packageManagers) {
			try {
				...
				this.log.warn('Failed to create package manager in batch', { name: pm.name, error });
			}
		}

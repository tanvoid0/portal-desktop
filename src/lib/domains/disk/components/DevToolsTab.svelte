<script lang="ts">
  ...
  async function runScan() {
    ...
    try {
      const res = await invoke<DevCleanerScan>('scan_dev_cleaners', {
...
<p class="mb-4 max-w-2xl text-sm text-muted-foreground">
  Scans dev tools for reclaimable space — Docker, Podman, and optionally project build junk on
  selected drives or folders. Review every item before removal.
</p>

<div class="mb-4">
  <div class="mb-2 text-xs uppercase tracking-wide text-muted-foreground">
    Project scan roots (optional)
  </div>
    ...
    <CardContent class="px-4">
      <div class="mb-2 flex items-center justify-between text-sm">
        ...
        <span class="tabular-nums text-muted-foreground"
          >{counting ? fmtDuration(scanProgress.elapsedMs) : `${scanPct.toFixed(0)}%`}</span
        >
      </div>
...
{#if cleanProgress}
  {@const pct =
    cleanProgress.total > 0 ? Math.min(100, (cleanProgress.done / cleanProgress.total) * 100) : 0}
  <Card class="mb-4 gap-0 py-4">
    <CardContent class="px-4">
      <div class="mb-2 flex items-center justify-between text-sm">
        <span class="text-foreground">
          Cleaning<span class="text-muted-foreground">
            · {cleanProgress.done} / {cleanProgress.total}</span
          >
        </span>
  ...
  <div class="space-y-3">
    {#each scanResult.groups as g (g.cleanerId)}
      <Card class="gap-0 overflow-hidden py-0">
        <div
          class="divider-edge-b divider-edge-full flex items-center gap-2.5 bg-muted/40 px-3 py-2.5"
        >
        ...
        {#if !collapsed.has(g.cleanerId) && g.available}
          {#if g.items.length === 0}
            <CardContent class="py-6 text-center text-sm text-muted-foreground"
              >Nothing to reclaim.</CardContent
            >
            ...
            {#each projectSubgroups(g.items) as sub (sub.label)}
              ...
              <div class="divider-edge-b divider-edge-full last:[&::after]:hidden">
                <div class="flex items-center gap-2 bg-muted/20 px-3 py-2">
                  ...
                  <span class="break-all font-mono text-xs text-foreground" title={sub.label}
                    >{sub.label}</span
                  >
                  <span class="ml-auto text-sm font-semibold tabular-nums"
                    >{fmtBytes(sub.bytes)}</span
                  >
                </div>
                      ...
                      {#each sub.items as item (item.id)}
                        <TableRow>
                          ...
                          <TableCell class="font-mono text-xs"
                            ><span class="text-muted-foreground">…\</span>{item.kind}</TableCell
                          >
                          <TableCell>
                            <Badge variant="outline" class={RISK_BADGE[item.risk] ?? ''}
                              >{item.risk}</Badge
                            >
                          </TableCell>
                          <TableCell
                          ...
                          <TableCell class="whitespace-nowrap text-right"
                            >{fmtBytes(item.sizeBytes)}</TableCell
                          >
                        </TableRow>
                      {/each}
                    </TableBody>
              ...
              <TableBody>
                {#each g.items as item (item.id)}
                  <TableRow>
                    ...
                    <TableCell class="break-all font-mono text-xs text-foreground"
                      >{item.path}</TableCell
                    >
                    <TableCell>
                      <Badge variant="outline" class={KIND_BADGE[item.kind] ?? ''}
                        >{kindLabel(item.kind)}</Badge
                      >
                    </TableCell>
                    <TableCell>
                      <Badge variant="outline" class={RISK_BADGE[item.risk] ?? ''}

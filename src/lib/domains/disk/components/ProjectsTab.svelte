<script lang="ts">
  ...
  import { fmtBytes, fmtDuration, KIND_BADGE, DISK_STAT_GRID } from '../utils';
  ...
  const selectedBytes = $derived(
    allTemps.filter((t) => selected.has(t.id)).reduce((a, t) => a + t.sizeBytes, 0)
  );
...
<div class="mb-4 flex items-center gap-2">
  ...
  <Button onclick={() => runScan()} disabled={busy || !root}
    >{busy ? 'Working…' : 'Scan projects'}</Button
  >
  {#if busy}
    ...
    <CardContent class="px-4">
      <div class="mb-2 flex items-center justify-between text-sm">
        ...
        <span class="tabular-nums text-muted-foreground"
          >{counting ? fmtDuration(progress.elapsedMs) : `${scanPct.toFixed(0)}%`}</span
        >
      </div>
      {#if counting}
    ...
    <Card class="gap-0 py-4">
      <CardContent class="px-4">
        ...
        <div class="mt-1 text-xs text-muted-foreground">
          {selected.size} dir{selected.size === 1 ? '' : 's'}
        </div>
      </CardContent>
    </Card>
  </div>
      ...
      <Card class="gap-0 overflow-hidden py-0">
        ...
        >
          ...
          <Button
            ...
            onclick={() => toggleCollapse(p.root)}>{collapsed.has(p.root) ? '▸' : '▾'}</Button
          >
          <span class="break-all font-mono text-xs text-foreground" title={p.root}
            >{rel(p.root, scanResult.root)}</span
          >
          ...
          <div class="ml-auto flex items-center gap-3 whitespace-nowrap">
            <span class="text-xs text-muted-foreground"
              >{p.temps.length} dir{p.temps.length === 1 ? '' : 's'}</span
            >
            <span class="text-sm font-semibold tabular-nums text-foreground"
              >{fmtBytes(p.totalBytes)}</span
            >
          </div>
        </div>
              ...
              {#each p.temps as t (t.id)}
                <TableRow>
                  ...
                  <TableCell class="break-all font-mono text-xs"
                    ><span class="text-muted-foreground">…\</span>{t.tempKind}</TableCell
                  >
                  <TableCell class="whitespace-nowrap text-xs text-muted-foreground"
                    >{t.fileCount.toLocaleString()} files</TableCell
                  >

...
const FRAMEWORK_COLORS: Record<string, string> = {
  ...
  FastAPI: 'bg-green-100 text-green-800 dark:bg-green-900/50 dark:text-green-200',
  Django: 'bg-green-100 text-green-800 dark:bg-green-900/50 dark:text-green-200',
  ...
  Spring: 'bg-green-100 text-green-800 dark:bg-green-900/50 dark:text-green-200',
  'ASP.NET': 'bg-blue-100 text-blue-800 dark:bg-blue-900/50 dark:text-blue-200',
  ...
  'React Native': 'bg-blue-100 text-blue-800 dark:bg-blue-900/50 dark:text-blue-200',
  Electron: 'bg-blue-100 text-blue-800 dark:bg-blue-900/50 dark:text-blue-200',
  Tauri: 'bg-orange-100 text-orange-800 dark:bg-orange-900/50 dark:text-orange-200',
};
...
export function getProjectFramework(project: Project): string | undefined {
  const extended = project as ProjectWithBackendFields;
  return project.metadata?.framework ?? extended.framework;
}

export function getProjectPackageManager(project: Project): string | undefined {
  const extended = project as ProjectWithBackendFields;
  return project.metadata?.dependencies?.packageManager ?? extended.package_manager;
}
...
export function getFrameworkIcon(framework: string | null | undefined): string {
...
export function getFrameworkIconBackground(framework: string | null | undefined): string {
...
export function getPackageManagerIcon(
  ...
  return PACKAGE_MANAGER_ICONS[packageManager.toLowerCase()] ?? 'devicon-devicon-plain';
}

...
class ProjectIconRegistry {
  ...
  private resolveByIds(kind: EntityKind, ids: number[] | undefined): ResolvedTechIcon[] {
  ...
  private resolveByName(kind: EntityKind, name: string | undefined): ResolvedTechIcon | undefined {
  ...
  resolvePackageManagers(project: Project): ResolvedTechIcon[] {
    const fromIds = this.resolveByIds('packageManager', project.package_manager_ids);
    if (fromIds.length > 0) return fromIds;

    const resolved = this.resolveByName('packageManager', getProjectPackageManager(project));
    return resolved ? [resolved] : [];
  }

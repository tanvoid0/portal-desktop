...
function pickPack(names: string[]) {
  const slugs = names.map((n) => frameworkToSlug(n)).filter((s): s is string => !!s);
  const priority = ['nextjs', 'nuxt', 'react', 'vue', 'svelte', 'angular', 'django', 'rust'];
...
describe('frameworkStarterService', () => {
  ...
  it('returns nextjs install/dev/build pipeline defs', () => {
    ...
    expect(pack?.pipelines.map((p) => p.category)).toEqual(['install', 'dev', 'build']);
  });

  it('keeps pnpm build shorthand', () => {
    expect(normalizePackageScriptCommand('pnpm', 'pnpm build', 'build')).toBe('pnpm build');
  });

  it('normalizes script bodies to pnpm build', () => {
    expect(normalizePackageScriptCommand('pnpm', 'next build --webpack', 'build')).toBe(
      'pnpm build'
    );
  });

  it('uses npm run build for npm projects', () => {
    expect(normalizePackageScriptCommand('npm', 'next build', 'build')).toBe('npm run build');
  });

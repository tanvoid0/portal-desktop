...
import type { CreatePipelineRequest, ExecutionContext, Pipeline, PipelineStep } from '../types';
...
export function resolvePackageManagerName(project: Project): string | undefined {
...
function findStarterPackForFrameworks(frameworkNames: string[]): FrameworkStarterPack | null {
...
export function resolveCommands(project: Project, def: StarterPipelineDef): string {
...
export function getStarterPackForProject(
  ...
  const provisionedKeys = existingPipelines.map((p) => p.presetKey).filter((k): k is string => !!k);
...
export async function autoProvisionStarterPipelines(
  ...
  log.info('autoProvisionStarterPipelines is a no-op; use the Actions catalog instead');
  return [];
}

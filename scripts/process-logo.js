...
try {
  ...
  try {
    ...
    execSync(`iconutil -c icns "${tempIcnsDir}" -o "${join(iconsDir, 'icon.icns')}"`, {
      stdio: 'inherit',
    });
    ...
    console.log(
      '    ⚠ icon.icns creation skipped (iconutil not available - run on macOS to generate)'
    );
  ...
  try {
    ...
    if (stats.size > 0) {
      ...
      execSync(`convert "${logoPath}" -resize 512x512\> "${tempPath}"`, {
        stdio: 'inherit',
      });

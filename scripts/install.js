...
async function installWindows() {
  ...
  if (!checkCommand("python --version", "Python")) {
    ...
    log(
      "   1. Microsoft Store: https://www.microsoft.com/store/apps/9P7QFQMJRFP7",
      "blue",
    );
  ...
  if (!msbuildFound) {
    ...
    log(
      '⚠️  Note: If you encounter "Spectre-mitigated libraries" errors:',
      "yellow",
    );
    ...
    log(
      '   4. Search for "Spectre" and install "MSVC v143 - VS 2022 C++ x64/x86 Spectre-mitigated libs"',
      "blue",
    );
  }
...
async function installMacOS() {
  ...
  if (!checkCommand("xcode-select -p", "Xcode Command Line Tools")) {
    ...
    try {
      ...
      log(
        "⏳ Please complete the Xcode installation dialog and run this script again",
        "yellow",
      );
      return false;
    } catch (error) {
...
async function installLinux() {
  ...
  if (distro === "debian") {
    ...
    try {
      ...
      execSync(
        "sudo apt-get install -y build-essential python3 python3-pip curl",
        { stdio: "inherit" },
      );
    } catch (error) {
    ...
    try {
      if (checkCommand("dnf --version", "DNF")) {
        execSync('sudo dnf groupinstall -y "Development Tools"', {
          stdio: "inherit",
        });
        execSync("sudo dnf install -y python3 python3-pip curl", {
          stdio: "inherit",
        });
      } else {
        execSync('sudo yum groupinstall -y "Development Tools"', {
          stdio: "inherit",
        });
        execSync("sudo yum install -y python3 python3-pip curl", {
          stdio: "inherit",
        });
      }
    } catch (error) {
    ...
    try {
      execSync("sudo pacman -S --noconfirm base-devel python curl", {
        stdio: "inherit",
      });
    } catch (error) {
      log("❌ Failed to install build tools", "red");
    ...
    log(
      "⚠️  Unsupported Linux distribution. Please install manually:",
      "yellow",
    );
...
function checkNodeVersion() {
  try {
    ...
    if (nodeVersion !== expectedVersion) {
      log(
        `⚠️  Warning: Expected Node.js ${expectedVersion}, but found ${nodeVersion}`,
        "yellow",
      );
      log(
        "   This may cause build issues. Consider using nvm to switch versions:",
        "yellow",
      );
...
async function installNodeDependencies() {
  ...
  if (!versionOk) {
    log(
      "⚠️  Continuing with installation despite version mismatch...",
      "yellow",
    );
  }

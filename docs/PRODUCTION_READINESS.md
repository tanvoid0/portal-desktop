...
**Fix:** clear existing debt, then remove `continue-on-error`; or freeze current violations to an allowlist so only _new_ ones gate.
...
_Note for maintainers: the P0 telemetry endpoint and the credential-key derivation are the two items to fix today — one leaks live data, the other makes credential encryption cosmetic. Everything else can be scheduled._

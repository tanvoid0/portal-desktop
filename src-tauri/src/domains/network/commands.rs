...
/// Passcodes are a single shared secret generated by the host, not bound to any
/// one device. The device that later presents the passcode (e.g. a phone that
/// scanned the QR code) registers itself as pending on verify. All passcode
/// store operations use this fixed key.
const SHARED_PASSCODE_KEY: &str = "__shared__";
...
pub async fn generate_device_passcode(
    _request: GeneratePasscodeRequest,
    _db: tauri::State<'_, Arc<DatabaseManager>>,
) -> Result<GeneratePasscodeResponse, String> {
    // The host generates a single shared passcode. It is NOT tied to the host's
    // own device_id and does NOT create a pending approval row — the requesting
    // device (phone) registers itself when it verifies the passcode.
    let store = get_passcode_store();
    let passcode = store.generate_passcode();
    store
        .store_passcode(SHARED_PASSCODE_KEY.to_string(), passcode.clone())
        .await;

    log_info!("device_auth", "Generated new shared device passcode");
...
/// Verify the shared passcode and register the requesting device as pending.
/// The actual approval happens later via the approve_device command (host UI).
#[command]
pub async fn verify_device_passcode(
    ...
    // Verify against the single shared passcode generated by the host.
    let store = get_passcode_store();
    if !store
        .verify_passcode(SHARED_PASSCODE_KEY, &request.passcode)
        .await
    {
    ...
    // Passcode is valid: register (or refresh) this device as a pending approval
    // so it shows up in the host's approval dialog. We intentionally keep the
    // shared passcode in the store until it expires so multiple devices can join.
    let conn = db.get_connection_clone();
    let passcode_expires_at = (Utc::now() + Duration::minutes(5)).into();
    ...
    if let Some(existing_device) = existing {
        ...
        device.device_name = Set(request.device_name.clone());
        device.device_info = Set(request.device_info.clone());
        device.passcode = Set(hash_passcode(&request.passcode));
        device.passcode_expires_at = Set(passcode_expires_at);
        device.updated_at = Set(Utc::now().into());
        device_approval::Entity::update(device)
            .exec(&conn)
            .await
            .map_err(|e| format!("Failed to update device approval: {}", e))?;
    } else {
        let approval = device_approval::ActiveModel {
            ...
            device_info: Set(request.device_info.clone()),
            passcode: Set(hash_passcode(&request.passcode)),
            passcode_expires_at: Set(passcode_expires_at),
            approved: Set(false),
        ...
        device_approval::Entity::insert(approval)
            .exec(&conn)
            .await
            .map_err(|e| format!("Failed to store device approval: {}", e))?;
    }

    log_info!(
        "device_auth",
        "Passcode verified, device_id {} registered as pending",
        request.device_id
    );

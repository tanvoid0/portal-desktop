use crate::domains::package_managers::entities::{SuggestedPackageManager, PackageManagerGroup};
...
pub async fn get_suggested_package_managers(
    ...
    for pattern in &learned_patterns {
        if let Ok(data) = serde_json::from_str::<serde_json::Value>(&pattern.pattern_data) {
            if let Some(pm) = data.get("package_manager").and_then(|v| v.as_str()) {
                ...
                if let Some((_, freq)) = learned_package_managers.iter_mut().find(|(name, _)| name == pm) {
    ...
    let groups = vec![
        PackageManagerGroup {
            ...
            package_managers: vec![
                SuggestedPackageManager { name: "npm".to_string(), icon: "logos:npm-icon".to_string(), category: "JavaScript/TypeScript".to_string() },
                SuggestedPackageManager { name: "yarn".to_string(), icon: "logos:yarn".to_string(), category: "JavaScript/TypeScript".to_string() },
                SuggestedPackageManager { name: "pnpm".to_string(), icon: "logos:pnpm".to_string(), category: "JavaScript/TypeScript".to_string() },
                SuggestedPackageManager { name: "bun".to_string(), icon: "logos:bun".to_string(), category: "JavaScript/TypeScript".to_string() },
            ],
        },
        PackageManagerGroup {
            category: "Python".to_string(),
            package_managers: vec![
                SuggestedPackageManager { name: "pip".to_string(), icon: "logos:python".to_string(), category: "Python".to_string() },
                SuggestedPackageManager { name: "pipenv".to_string(), icon: "logos:python".to_string(), category: "Python".to_string() },
                SuggestedPackageManager { name: "poetry".to_string(), icon: "logos:poetry".to_string(), category: "Python".to_string() },
                SuggestedPackageManager { name: "conda".to_string(), icon: "logos:conda".to_string(), category: "Python".to_string() },
            ],
        },
        PackageManagerGroup {
            category: "Rust".to_string(),
            package_managers: vec![
                SuggestedPackageManager { name: "cargo".to_string(), icon: "logos:rust".to_string(), category: "Rust".to_string() },
            ],
        },
        PackageManagerGroup {
            category: "Go".to_string(),
            package_managers: vec![
                SuggestedPackageManager { name: "go mod".to_string(), icon: "logos:go".to_string(), category: "Go".to_string() },
            ],
        },
        PackageManagerGroup {
            category: "Java".to_string(),
            package_managers: vec![
                SuggestedPackageManager { name: "maven".to_string(), icon: "logos:maven".to_string(), category: "Java".to_string() },
                SuggestedPackageManager { name: "gradle".to_string(), icon: "logos:gradle".to_string(), category: "Java".to_string() },
            ],
        },
        PackageManagerGroup {
            category: "PHP".to_string(),
            package_managers: vec![
                SuggestedPackageManager { name: "composer".to_string(), icon: "logos:composer".to_string(), category: "PHP".to_string() },
            ],
        },
        PackageManagerGroup {
            category: "Ruby".to_string(),
            package_managers: vec![
                SuggestedPackageManager { name: "bundler".to_string(), icon: "logos:ruby".to_string(), category: "Ruby".to_string() },
                SuggestedPackageManager { name: "gem".to_string(), icon: "logos:ruby".to_string(), category: "Ruby".to_string() },
            ],
        },
        PackageManagerGroup {
            category: ".NET".to_string(),
            package_managers: vec![
                SuggestedPackageManager { name: "nuget".to_string(), icon: "logos:nuget-icon".to_string(), category: ".NET".to_string() },
                SuggestedPackageManager { name: "dotnet".to_string(), icon: "logos:dotnet".to_string(), category: ".NET".to_string() },
            ],
        },
    ];
    ...
    if !learned_package_managers.is_empty() {
        ...
        if !recommended_package_managers.is_empty() {
            ...
            groups_with_recommended.insert(0, PackageManagerGroup {
...
pub async fn create_package_manager(
    ...
    if let Ok(Some(_)) = PackageManagerRepository::get_by_name(&conn, &name).await {
        return Err(format!("Package manager with name '{}' already exists", name));
    }
...
pub async fn update_package_manager(
    ...
    if let Some(ref new_name) = name {
        if let Ok(Some(existing)) = PackageManagerRepository::get_by_name(&conn, new_name).await {
            if existing.id != id {
                return Err(format!("Package manager with name '{}' already exists", new_name));
            }
        }
    }

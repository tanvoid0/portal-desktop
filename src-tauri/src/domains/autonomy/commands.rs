...
pub async fn evaluate_autonomous_action(
    ...
    let result = {
        ...
        let service = services
            .get("default")
            .ok_or("Failed to get autonomy service")?;
...
#[command]
pub async fn get_autonomy_level() -> Result<String, String> {
    let service_map = get_services_map();
...
#[command]
pub async fn set_autonomy_level(level: String) -> Result<(), String> {
    let level = match level.to_lowercase().as_str() {
        "observation" => {
            crate::domains::autonomy::services::autonomy_service::AutonomyLevel::Observation
        }
        "conservative" => {
            crate::domains::autonomy::services::autonomy_service::AutonomyLevel::Conservative
        }
        "balanced" => crate::domains::autonomy::services::autonomy_service::AutonomyLevel::Balanced,
        "aggressive" => {
            crate::domains::autonomy::services::autonomy_service::AutonomyLevel::Aggressive
        }
...
#[command]
pub async fn get_autonomy_enabled() -> Result<bool, String> {
    let service_map = get_services_map();
    let services = service_map.lock().await;
    let service = services
        .get("default")
...
#[command]
pub async fn set_autonomy_enabled(enabled: bool) -> Result<(), String> {
    let service_map = get_services_map();
    let mut services = service_map.lock().await;
    let service = services
        .get_mut("default")
...
#[command]
pub async fn get_approval_stats() -> Result<Value, String> {
    let service_map = get_services_map();
    let services = service_map.lock().await;
    let service = services
        .get("default")

...
use crate::domains::sdk::traits::package_manager::{Package, InstalledPackage, PackageDetails, PackageUpdate};
...
pub async fn package_manager_install(
    ...
    if let Some(manager) = factory.get_manager(&manager_name) {
        match manager.install_package(&package_id, version.as_deref()).await {
            Ok(_) => Ok(format!("Successfully installed {} via {}", package_id, manager_name)),
...
pub async fn package_manager_upgrade(
    ...
    if let Some(manager) = factory.get_manager(&manager_name) {
        match manager.upgrade_package(&package_id).await {
            Ok(_) => Ok(format!("Successfully upgraded {} via {}", package_id, manager_name)),
...
pub async fn package_manager_uninstall(
    ...
    if let Some(manager) = factory.get_manager(&manager_name) {
        match manager.uninstall_package(&package_id).await {
            Ok(_) => Ok(format!("Successfully uninstalled {} via {}", package_id, manager_name)),
...
pub async fn package_manager_info(manager_name: String) -> Result<serde_json::Value, String> {
    ...
    if let Some(manager) = factory.get_manager(&manager_name) {
        ...
        let version = manager.get_version().await.unwrap_or_else(|_| "unknown".to_string());

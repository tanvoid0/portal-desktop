...
impl BinaryDownloader {
    ...
    pub async fn download_version(
        ...
        tokio::fs::create_dir_all(&self.cache_dir)
            .await
            .map_err(|e| {
                SDKError::ManagerNotFound(format!("Failed to create cache directory: {}", e))
            })?;

        // Download with progress tracking
        self.download_with_progress(download_url, &cache_path, progress_sender)
            .await?;
    ...
    async fn download_with_progress(
        ...
        let response =
            self.client.get(url).send().await.map_err(|e| {
                SDKError::ManagerNotFound(format!("Failed to start download: {}", e))
            })?;
        ...
        while let Some(chunk) = stream.next().await {
            let chunk =
                chunk.map_err(|e| SDKError::ManagerNotFound(format!("Download error: {}", e)))?;
                ...
                || downloaded - last_downloaded > 1024 * 1024
            {
                ...
                progress_sender
                    ...
                    .map_err(|_| {
                        SDKError::ManagerNotFound("Failed to send progress".to_string())
                    })?;
    ...
    pub async fn verify_checksum(
        ...
        let mut file = std::fs::File::open(file_path).map_err(|e| {
            SDKError::ManagerNotFound(format!("Failed to open file for checksum: {}", e))
        })?;

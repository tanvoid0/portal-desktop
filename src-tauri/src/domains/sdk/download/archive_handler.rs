...
impl ArchiveHandler {
    ...
    pub async fn extract_archive(
        ...
        tokio::fs::create_dir_all(destination).await.map_err(|e| {
            SDKError::ManagerNotFound(format!("Failed to create destination directory: {}", e))
        })?;
        ...
        match extension {
            "gz" | "tgz" => {
                self.extract_tar_gz(archive_path, destination, progress_sender)
                    .await
            }
            "xz" => {
                self.extract_tar_xz(archive_path, destination, progress_sender)
                    .await
            }
            "zip" => {
                self.extract_zip(archive_path, destination, progress_sender)
                    .await
            }
    ...
    async fn extract_zip(
        ...
        archive.extract(destination).map_err(|e| {
            SDKError::ManagerNotFound(format!("Failed to extract zip archive: {}", e))
        })?;
    ...
    async fn move_child_dir_to_parent(&self, destination: &PathBuf) -> Result<(), SDKError> {
        let mut entries = tokio::fs::read_dir(destination).await.map_err(|e| {
            SDKError::ManagerNotFound(format!("Failed to read destination directory: {}", e))
        })?;

        let mut child_dirs = Vec::new();
        while let Some(entry) = entries.next_entry().await.map_err(|e| {
            SDKError::ManagerNotFound(format!("Failed to read directory entry: {}", e))
        })? {
            let path = entry.path();
            if path.is_dir() {
        ...
        if child_dirs.len() == 1 {
            ...
            tokio::fs::rename(child_dir, &temp_dir).await.map_err(|e| {
                SDKError::ManagerNotFound(format!("Failed to move child directory: {}", e))
            })?;

            // Move contents from temp to destination
            let mut temp_entries = tokio::fs::read_dir(&temp_dir).await.map_err(|e| {
                SDKError::ManagerNotFound(format!("Failed to read temp directory: {}", e))
            })?;

            while let Some(entry) = temp_entries.next_entry().await.map_err(|e| {
                SDKError::ManagerNotFound(format!("Failed to read temp entry: {}", e))
            })? {
                ...
                tokio::fs::rename(&entry_path, &dest_path)
                    .await
                    .map_err(|e| {
                        SDKError::ManagerNotFound(format!("Failed to move entry: {}", e))
                    })?;
            }

            // Remove temp directory
            tokio::fs::remove_dir(&temp_dir).await.map_err(|e| {
                SDKError::ManagerNotFound(format!("Failed to remove temp directory: {}", e))
            })?;
        }
    ...
    pub async fn create_activation_links(
        ...
        tokio::fs::create_dir_all(env_dir).await.map_err(|e| {
            SDKError::ManagerNotFound(format!("Failed to create env directory: {}", e))
        })?;
        ...
        if link_path.exists() {
            tokio::fs::remove_file(&link_path).await.map_err(|e| {
                SDKError::ManagerNotFound(format!("Failed to remove existing link: {}", e))
            })?;
        }
        ...
        if cfg!(target_os = "windows") {
            ...
            #[cfg(unix)]
            {
                std::os::unix::fs::symlink(&binary_path, &link_path).map_err(|e| {
                    SDKError::ManagerNotFound(format!("Failed to create symlink: {}", e))
                })?;
            }
        }

use crate::domains::sdk::managers::{
    NvmManager, PhpenvManager, PyenvManager, RbenvManager, RustupManager, SdkmanManager,
};
use crate::domains::sdk::traits::sdk_manager::SDKManager;
...
impl SDKManagerFactory {
    ...
    pub async fn detect_installed_managers(
        &self,
    ) -> Result<Vec<HashMap<String, String>>, SDKError> {
        let mut installed = Vec::new();

        println!(
            "[SDKManagerFactory] Starting detection of {} managers",
            self.managers.len()
        );
                ...
                Ok(true) => {
                    ...
                    match manager.get_manager_version().await {
                        Ok(version) => {
                            ...
                            println!(
                                "[SDKManagerFactory] Added manager {} with version {}",
                                name, version
                            );
                            installed.push(info);
                        }
                        Err(e) => {
                            println!(
                                "[SDKManagerFactory] Failed to get version for {}: {}",
                                name, e
                            );
                        }
                    }
        ...
        println!(
            "[SDKManagerFactory] Detection complete. Found {} installed managers",
            installed.len()
        );
        Ok(installed)
    }
    ...
    pub async fn get_all_manager_info(
        &self,
    ) -> Result<HashMap<String, HashMap<String, String>>, SDKError> {
        let mut info = HashMap::new();
...
impl SDKManagerFactory {
    ...
    pub async fn switch_version_for_project(
        &self,
        manager_name: &str,
        version: &str,
        project_path: &str,
        ...
        if let Some(manager) = self.get_manager(manager_name) {
            manager
                .switch_version_for_project(version, project_path)
                .await
        } else {
        ...
        if let Some(manager) = self.get_manager(manager_name) {
            if manager.supports_installation() {
                ...
                Err(SDKError::ManagerNotFound(format!(
                    "Installation not supported for {}",
                    manager_name
                )))
            }
        } else {

...
#[async_trait]
impl SDKManager for RbenvManager {
    fn name(&self) -> &'static str { "rbenv" }
    fn display_name(&self) -> &'static str { "Ruby Version Manager" }
    fn sdk_type(&self) -> &'static str { "ruby" }
    ...
    // === Help & Validation ===
    async fn get_help(&self) -> Result<String, SDKError> { Ok("Rbenv help - not implemented".to_string()) }
    async fn get_usage_examples(&self) -> Result<Vec<String>, SDKError> { Ok(vec!["rbenv install 3.0.0".to_string()]) }
    async fn validate_setup(&self) -> Result<Vec<String>, SDKError> { Ok(vec!["Rbenv not implemented".to_string()]) }
...
#[async_trait]
impl SDKManagerDefaults for RbenvManager {}

#[async_trait]
impl SDKManagerHelpers for RbenvManager {}
...
use crate::domains::sdk::configs::{get_sdk_config, get_all_sdk_configs, SDKConfig, SDKCategory};
...
impl SDKConfigService {
    ...
    pub async fn get_sdks_by_category(category: SDKCategory) -> Result<Vec<ProcessedSDKConfig>, String> {
    ...
    pub async fn get_all_sdk_managers() -> Result<Vec<ProcessedSDKManager>, String> {
        let configs = get_all_sdk_configs();
        let mut managers_map: std::collections::HashMap<String, ProcessedSDKManager> = std::collections::HashMap::new();
        ...
        for config in configs {
            for m in config.sdk_managers {
                ...
                if !managers_map.contains_key(&m.id) {
                    ...
                    managers_map.insert(m.id.clone(), ProcessedSDKManager {
                        id: m.id,
                        name: m.name,
    ...
    async fn process_config(config: SDKConfig) -> ProcessedSDKConfig {
        ...
        let tabs: Vec<ProcessedTabConfig> = config.tabs
            .into_iter()
            .filter(|t| t.enabled)
        ...
        let supported_sources: Vec<String> = config.supported_sources
            .into_iter()
            .map(|s| match s {
                crate::domains::sdk::configs::types::VersionSource::Static => "static".to_string(),
                crate::domains::sdk::configs::types::VersionSource::SdkManager => "sdk_manager".to_string(),
        ...
        let default_source = match config.default_source {
            Some(crate::domains::sdk::configs::types::VersionSource::Static) => "static".to_string(),
            Some(crate::domains::sdk::configs::types::VersionSource::SdkManager) => "sdk_manager".to_string(),
            Some(crate::domains::sdk::configs::types::VersionSource::System) => "system".to_string(),
            Some(crate::domains::sdk::configs::types::VersionSource::Custom) => "custom".to_string(),
        ...
        // Check service status for database/server SDKs
        let (service_running, service_port) = if matches!(config.category, SDKCategory::Database | SDKCategory::Server | SDKCategory::AI) {
    ...
    async fn check_sdk_installation(detection: &crate::domains::sdk::configs::types::DetectionMethod) -> (bool, Option<String>) {
        use std::process::Command;
    ...
    async fn check_service_status(service_config: &Option<serde_json::Value>) -> (Option<bool>, Option<u16>) {
        ...
        match binary {
            "java" => {
                ...
                if let Some(first_line) = raw_output.lines().next() {
                    ...
                    if let Some(captures) = regex::Regex::new(r#"version\s+"?([0-9]+\.[0-9]+\.[0-9]+)"#)
                        .ok()
                ...
                raw_output.lines().next().unwrap_or(raw_output).trim().to_string()
            },
            _ => {

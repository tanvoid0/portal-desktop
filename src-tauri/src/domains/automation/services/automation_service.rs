...
impl AutomationService {
    ...
    pub async fn trigger_workflow(
        ...
        if !response.status().is_success() {
            return Err(format!(
                "Workflow trigger failed with status: {}",
                response.status()
            ));
        }

        let result: WorkflowResult = response
            .json()
            .await
    ...
    pub async fn get_workflow_status(
        ...
        let execution: WorkflowExecution = response
            .json()
            .await
    ...
    pub async fn list_available_workflows(&self) -> Result<Vec<AvailableWorkflow>, String> {
        ...
        let workflows: Vec<AvailableWorkflow> = response
            .json()
            .await
    ...
    pub async fn get_suggested_workflows(
        ...
        let ai_workflows = vec!["ai_code_analysis", "ai_suggestions", "ai_code_review"];
            ...
            let matches_package_manager = package_manager
                .map(|pm| {
                    workflow
                        .package_manager
                        .as_ref()
                        .map_or(false, |wpm| wpm.contains(pm))
                })
                .unwrap_or(true);

            // Always include AI workflows
            let is_ai_workflow = ai_workflows
                .iter()
                .any(|&ai| workflow.name.to_lowercase().contains(ai));
